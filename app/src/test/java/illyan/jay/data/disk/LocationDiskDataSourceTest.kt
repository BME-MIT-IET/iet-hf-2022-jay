...
class LocationDiskDataSourceTest : TestBase() {
	...
	private val roomLocation = RoomLocation(4, 2, 4.0, 4.0, 4f, location.time.toInstant().toEpochMilli(), 4f, 4f, 4f, 4f, 4.0, 4f)
	private val locations = listOf(
		...
		DomainLocation(2, LatLng(2.0, 2.0), 2f, 1, Date.from(Instant.ofEpochMilli(random.nextLong(now.toEpochMilli() - TimeUtils.DAY_IN_MILLIS, now.toEpochMilli()))), 2f, 2f, 2f, 2.0, 2f, 2f),
		DomainLocation(3, LatLng(3.0, 3.0), 3f, 1, Date.from(Instant.ofEpochMilli(random.nextLong(now.toEpochMilli() - TimeUtils.DAY_IN_MILLIS, now.toEpochMilli()))), 3f, 3f, 3f, 3.0, 3f, 3f),
		location
	)
	private val roomLocations = listOf(
		RoomLocation(1, 1, 1.0, 1.0, 1f, locations[0].time.toInstant().toEpochMilli(), 1f, 1f, 1f, 1f, 1.0, 1f),
		RoomLocation(2, 1, 2.0, 2.0, 2f, locations[1].time.toInstant().toEpochMilli(), 2f, 2f, 2f, 2f, 2.0, 2f),
		RoomLocation(3, 1, 3.0, 3.0, 3f, locations[2].time.toInstant().toEpochMilli(), 3f, 3f, 3f, 3f, 3.0, 3f),
		roomLocation
	)
	...
	fun `Save multiple location data`() {
		...
		verify(exactly = 1) { mockedDao.upsertLocations(locations.map(DomainLocation::toRoomModel)) }
	}
}